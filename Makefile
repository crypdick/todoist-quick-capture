...
ifeq ($(OS),win32)
    ...
    TEST_COMMAND := set PYTHONPATH=$(PYTHONPATH) && poetry run pytest -c pyproject.toml --cov-report=todoist_quick_capture --cov=hooks tests/
else
	PYTHONPATH := `pwd`
    TEST_COMMAND := PYTHONPATH=$(PYTHONPATH) poetry run pytest -c pyproject.toml --cov-report=html --cov=todoist_quick_capture tests/
endif


#* Docker variables
IMAGE := todoist_quick_capture
VERSION := latest
